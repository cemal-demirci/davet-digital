...
import { Cake, Calendar, MapPin, Clock, Camera, Users, Loader, AlertCircle } from 'lucide-react'
...
const BirthdaySite = () => {
  ...
  useEffect(() => {
    const fetchTenant = async () => {
      try {
        ...
        if (!tenantSlug) {
          setError('Doğum günü sitesi bulunamadı.')
          setLoading(false)
          return
        }
        ...
        setError(error.response?.data?.error || 'Doğum günü sitesi yüklenirken bir hata oluştu.')
        setLoading(false)
      }
    }
  ...
  // Countdown timer
  useEffect(() => {
    if (!tenant?.birthDate) return

    const updateCountdown = () => {
      const birthDate = new Date(tenant.birthDate)
      const now = new Date()
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-orange-100 to-red-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Doğum günü sitesi yükleniyor...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-orange-100 to-red-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md text-center">
          ...
          <a href="https://www.davet.digital" className="inline-block px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-600 text-white rounded-full font-semibold hover:shadow-lg transition-all">
            Ana Sayfaya Dön
          </a>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-red-50">
      {/* Hero Section */}
      <div className="relative overflow-hidden bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 text-white py-32">
        <div className="absolute inset-0 bg-black/20"></div>
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          <div className="flex justify-center mb-8">
            <Cake className="w-20 h-20 animate-pulse" />
          </div>
          <h1 className="text-6xl md:text-7xl font-script mb-6">
            {tenant.celebrantName}
          </h1>
      ...
      <div className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-4xl font-script text-center text-transparent bg-clip-text bg-gradient-to-r from-yellow-600 to-orange-600 mb-12">
            Partiye Kalan Süre
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div className="bg-gradient-to-br from-yellow-500 to-orange-600 rounded-2xl p-6 text-center text-white shadow-xl animate-pulse">
      ...
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 text-white py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <Cake className="w-12 h-12 text-yellow-500 mx-auto mb-4" />
          <p className="text-lg mb-2">{tenant.celebrantName}</p>
          <p className="text-gray-400 text-sm">
            Bu doğum günü sitesi <a href="https://www.davet.digital" className="text-yellow-400 hover:text-yellow-300">Davet Digital</a> ile oluşturulmuştur
          </p>
        </div>
      </div>
    </div>
